...
# 4.  Main
###############################################################################
async def main(ticker, expiry, data_type = 1, up_level= 7, down_level = 7, csv_update = 'N'):
    ...
    try:
        ...
        results = await batch_data(ib, BATCH_SIZE, expiry, ticker, centre_price, up_level, down_level)
        ...
        if csv_update == 'Y' or csv_update == 'YES':
            append_oi_data(results, ticker=ticker)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.ticker.upper(), args.expiry, args.data, args.up_level, args.down_level, args.csv))